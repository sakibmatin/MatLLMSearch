...
def run_analyze(args) -> Dict[str, Any]:
    ...
    if hasattr(args, 'output') and args.output:
        ...
        if hasattr(args, 'save_label') and args.save_label:
            output_subdir = args.save_label
        else:
            output_subdir = args._output_dir_model_name

        output_path = Path(log_dir) / output_subdir
...
def _generate_structures_via_csg(args) -> list:
    ...
    # Use the actual output path from CSG results, or construct from log_dir and save_label
